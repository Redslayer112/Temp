...
def _verify_file_hash(filepath, expected_hash, failed_validations, validation_lock, ui):
    ...
    with ui_lock:
        ...
        ui.stdscr.clrtoeol()
        ui.print_colored(hash_verify_line, 2, "🔍 Verifying file integrity...", 'warning')
        ui.stdscr.refresh()
        ...
        for i, file_info in enumerate(dir_info['files'], 1):
            ...
            try:
                ...
                with ui_lock:
                    ...
                    if hash_valid:
                        ui.print_colored(hash_verify_line, 2, f"✅ Hash Verified: {file_info['path']}", 'success')
                    else:
                        ui.print_colored(hash_verify_line, 2, f"⚠️ Failed hash check: {file_info['path']}", 'error')
                    ui.stdscr.refresh()
